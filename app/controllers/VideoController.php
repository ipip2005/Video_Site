...
class VideoController extends \Controller
{
    ...
    public function postRatingBlade()
    {
        $vid = Input::get("vid");
        return View::make('blades/rating')->with(compact('vid'));
    }
    ...
    public function postRating()
    {
        ...
        if ($score < 1 || $score > 5 || ! Auth::check()) {
            return Response::json(array(
                "error" => "wrong request"
            ));
        }
        $video = Video::findOrFail($vid);
        if (DB::table("scores")->where("uid", "=", Auth::id())
            ->where("vid", "=", $vid)
            ->exists()) {
            DB::table("scores")->where("uid", "=", Auth::id())
                ->where("vid", "=", $vid)
                ->update(array(
                'score' => $score
            ));
    ...
    public function postShare(){
        $vid = Input::get("vid");
        if (DB::table('share')->where('uid','=',Auth::id())->where('vid','=',$vid)->exists()){
            return Response::json(array(
                "error" => "already shared"
            ));
        }
        DB::table('share')->insert(array('uid'=>Auth::id(),'vid'=>$vid,'gid'=>0));
        return Response::json(array(
            "success"=>"1"
        ));
    }
    ...
    public function getCreate()
    {
        ...
        if ($type != 'flv' && $type != 'ogg' && $type != 'mp4' && $type != 'mov' && $type != 'wmv' && $type != 'mpeg' && $type != 'f4v')
            return Response::json(array(
